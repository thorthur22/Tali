...
def _start_agent_service(agent_name: str) -> str:
    return _run_cli_args(["agent", "start", agent_name], agent_context=agent_name, timeout=60)


def _stop_agent_service(agent_name: str) -> str:
    return _run_cli_args(["agent", "stop", agent_name], agent_context=agent_name, timeout=60)
...
def _resume_agent_run(root: Path, agent_name: str) -> str:
    ...
    run = db.fetch_active_run()
    if not run:
        return "No active or blocked run found."
    if run["status"] == "blocked":
        db.update_run_status(str(run["id"]), status="active", current_task_id=run["current_task_id"], last_error=None)
        start_msg = _start_agent_service(agent_name)
        return f"Run {run['id']} resumed. {start_msg}"
    if run["status"] == "active":
        pid = _service_pid(root, agent_name)
        if not _is_agent_service_pid(root, agent_name, pid):
            start_msg = _start_agent_service(agent_name)
            return f"Run {run['id']} is active but no service was running. {start_msg}"
    return f"Run {run['id']} is already active."
...
def _create_agent(
    ...
    base = base_url.strip() or "http://localhost:11434"
    planner = planner_model.strip() or "llama3"
    responder = responder_model.strip() or planner
    embed = embedding_model.strip() or "nomic-embed-text"
    ...
    cfg = AppConfig(
        ...
        capabilities=[],
        planner_llm=LLMSettings(provider="ollama", model=planner, base_url=base, api_key=None),
        responder_llm=LLMSettings(provider="ollama", model=responder, base_url=base, api_key=None),
        embeddings=EmbeddingSettings(
            provider="ollama", model=embed, base_url=base, api_key=None, dim=1536
        ),
...
def _layout(title: str, body: str, notice: str = "", detail: str = "", active_nav: str = "dashboard") -> str:
    ...
    if notice:
        notice_html = (
            ...
            f"<h3>{html.escape(notice)}</h3>"
            f"<pre>{html.escape(detail or '')}</pre>"
            "</section>"
        )
...
<head>
  ...
  <style>
    :root {{
      --bg:#f2f5fa; --bg2:#e6edf7; --panel:#ffffff; --text:#192231; --muted:#5a6780;
      --line:#d4dce9; --accent:#0b66d6; --accent2:#0b4ea6; --ok:#117a46; --warn:#9c5b03;
    }}
    * {{ box-sizing:border-box; }}
    body {{ margin:0; font:14px/1.45 'Avenir Next', 'Segoe UI', sans-serif; color:var(--text); background:radial-gradient(1200px 600px at -10% -10%, var(--bg2), var(--bg)); }}
    .shell {{ display:grid; grid-template-columns:240px 1fr; min-height:100vh; }}
    .sidebar {{ background:linear-gradient(180deg, #0f2442, #102a4d); color:#e6eefb; padding:22px 16px; border-right:1px solid rgba(255,255,255,0.08); }}
    .brand {{ font-size:20px; font-weight:700; letter-spacing:0.03em; margin-bottom:18px; }}
    ...
    .notice {{ margin-bottom:14px; border-color:#91b4e8; }}
    ...
    .tabs a {{ border:1px solid var(--line); border-radius:8px; padding:6px 10px; background:#fff; }}
    input,textarea,select,button {{ width:100%; border:1px solid var(--line); border-radius:9px; padding:8px 10px; font:inherit; background:#fff; margin-top:6px; }}
    button {{ border:0; background:var(--accent); color:#fff; font-weight:600; cursor:pointer; }}
    button.secondary {{ background:#54627a; }}
    button.warn {{ background:#ad3c31; }}
...
def _render_agents(root: Path, notice: str, detail: str) -> str:
    ...
    for a in agents:
        ...
        rows.append(
            ...
            f"<td>{'running' if a.service_running else 'stopped'}</td>"
            ...
            f"<td><form method='post' action='/agent/{safe}/start'><button type='submit'>Start</button></form></td>"
            f"<td><form method='post' action='/agent/{safe}/stop'><button class='warn' type='submit'>Stop</button></form></td>"
            f"<td><form method='post' action='/agent/{safe}/resume'><button class='secondary' type='submit'>Resume Run</button></form></td>"
            f"<td><form method='post' action='/agent/{safe}/cancel'><button class='secondary' type='submit'>Cancel Run</button></form></td>"
            "</tr>"
        )
    ...
    body = f"""
<h1>Agents</h1>
<p class='muted'>Lifecycle and runtime control for all agents.</p>
<section class='panel'>
  <div class='toolbar'>
...
<div id='create-modal' class='modal-backdrop' onclick='if(event.target===this)closeCreateModal()'>
  <div class='modal'>
    ...
    <form method='post' action='/agents/create'>
      ...
      <label>Planner Model<input name='planner_model' value='llama3' /></label>
      <label>Responder Model<input name='responder_model' value='llama3' /></label>
      <label>Embedding Model<input name='embedding_model' value='nomic-embed-text' /></label>
...
def _render_activity(root: Path, notice: str, detail: str) -> str:
    ...
    for name in _list_agent_names(root):
        paths = _paths(root, name)
        db = _init_db(paths.db_path)
        for run in db.list_runs(limit=8):
            runs.append((name, run))
        try:
            logs = read_recent_logs(paths.logs_dir, limit=5)
        except Exception:
            logs = []
        if logs:
            joined = "\n".join(
                f"[{entry.get('timestamp', '?')}] {entry.get('event', '')} {entry.get('message', '')}" for entry in logs
            )
  ...
  <section class='panel'>
    ...
    <div class='actions'>
      <form method='post' action='/agent/{safe}/chat'>
        <label>Task Message
          <textarea name='message' rows='4' placeholder='Continue executing current task until complete, using tools as needed.'></textarea>
        </label>
        <button type='submit'>Run Agent Turn</button>
      </form>
      <form method='post' action='/agent/{safe}/start'><button type='submit'>Start Service</button></form>
      <form method='post' action='/agent/{safe}/stop'><button class='warn' type='submit'>Stop Service</button></form>
      <form method='post' action='/agent/{safe}/resume'><button class='secondary' type='submit'>Resume Run</button></form>
      <form method='post' action='/agent/{safe}/cancel'><button class='secondary' type='submit'>Cancel Run</button></form>
      <form method='post' action='/agent/{safe}/delete'><button class='warn' type='submit'>Delete Agent</button></form>
    </div>
  </section>
</div>
...
<section class='panel'>
  ...
  <div class='actions'>
    <form method='post' action='/agent/{_q(agent_name)}/resume'><button class='secondary' type='submit'>Resume Active Run</button></form>
    <form method='post' action='/agent/{_q(agent_name)}/cancel'><button class='secondary' type='submit'>Cancel Active Run</button></form>
  </div>
</section>
<section class='panel' style='margin-top:12px;'>
  <h2>Run History</h2>
...
def _render_agent_memory(root: Path, agent_name: str, notice: str, detail: str) -> str:
    ...
    commitments_rows = "".join(
        f"<tr><td>{html.escape(str(r['status']))}</td><td>{html.escape(str(r['title']))}</td><td>{html.escape(str(r['due_date'] or '-'))}</td></tr>"
        for r in commitments
    ) or "<tr><td colspan='3' class='muted'>No commitments.</td></tr>"
...
<div class='grid'>
  <section class='panel'>
    <h2>Commitments</h2>
    <div class='table-wrap'>
      <table><thead><tr><th>Status</th><th>Title</th><th>Due</th></tr></thead><tbody>{commitments_rows}</tbody></table>
    </div>
  </section>
  <section class='panel'>
    <h2>Skills</h2>
...
class _UIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        try:
            ...
            if path == "/":
                self._send_html(_render_dashboard(self.root_dir, self.notice, self.detail))
                return
            if path == "/agents":
                self._send_html(_render_agents(self.root_dir, self.notice, self.detail))
                return
            if path == "/activity":
                self._send_html(_render_activity(self.root_dir, self.notice, self.detail))
                return
            if path == "/models":
                self._send_html(_render_models(self.notice, self.detail))
                return
            if path == "/operations":
                self._send_html(_render_operations(self.root_dir, self.notice, self.detail, self.agent_context or None))
                return
            ...
            if path.startswith("/agent/"):
                ...
                if len(parts) >= 2:
                    ...
                    if not self._agent_exists(name):
                        self._send_html(_layout("Not found", "<h1>Agent not found</h1>", self.notice, self.detail, active_nav="agents"), status=404)
                        return
                    if len(parts) == 2:
                        self._send_html(_render_agent_overview(self.root_dir, name, self.notice, self.detail))
                        return
                    if len(parts) == 3 and parts[2] == "runs":
                        self._send_html(_render_agent_runs(self.root_dir, name, self.notice, self.detail))
                        return
                    if len(parts) == 3 and parts[2] == "patches":
                        self._send_html(_render_agent_patches(self.root_dir, name, self.notice, self.detail))
                        return
                    if len(parts) == 4 and parts[2] == "patches":
                        self._send_html(_render_patch_detail(self.root_dir, name, parts[3], self.notice, self.detail))
                        return
                    if len(parts) == 3 and parts[2] == "memory":
                        self._send_html(_render_agent_memory(self.root_dir, name, self.notice, self.detail))
                        return
                    if len(parts) == 3 and parts[2] == "inbox":
                        self._send_html(_render_agent_inbox(self.root_dir, name, self.notice, self.detail))
                        return

            self._send_html(_layout("Not found", "<h1>Not found</h1>", self.notice, self.detail, active_nav="dashboard"), status=404)
            ...
            if path.startswith("/agent/"):
                ...
                if len(parts) >= 2:
                    ...
                    if len(parts) == 3 and parts[2] == "chat":
                        ...
                        self._redirect(f"/agent/{_q(name)}")
                        return
                    ...
                    if len(parts) == 3 and parts[2] == "start":
                        self._set_notice(_start_agent_service(name))
                        _invalidate_cache(name)
                        self._redirect(f"/agents")
                        return

                    if len(parts) == 3 and parts[2] == "stop":
                        self._set_notice(_stop_agent_service(name))
                        _invalidate_cache(name)
                        self._redirect(f"/agents")
                        return
                    ...
                    if len(parts) == 5 and parts[2] == "patches":
                        ...
                        _invalidate_cache(name)
                        self._redirect(f"/agent/{_q(name)}/patches/{_q(proposal_id)}")
                        return
                    ...
                    if len(parts) == 5 and parts[2] == "inbox" and parts[4] == "read":
                        ...
                        _invalidate_cache(name)
                        self._redirect(f"/agent/{_q(name)}/inbox")
                        return
